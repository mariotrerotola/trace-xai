...
def compute_structural_stability(
    ...
    *,
    ...
    is_regression = explainer._task == "regression"
    ...
    for a, b in combinations(range(n_bootstraps), 2):
        if is_regression:
            # Use 1 - normalized MAE as agreement
            max_range = max(
                np.ptp(predictions_list[a]),
                np.ptp(predictions_list[b]),
                1e-10,
            )
            mae = np.mean(np.abs(predictions_list[a] - predictions_list[b]))
            agreements.append(1.0 - mae / max_range)
        else:
